...
<svelte:head>
    <title>Nueva torre — Portal PH</title>
</svelte:head>
...
<AppLayout>
    <div class="mb-6 flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-semibold text-slate-900">Nueva torre</h1>
            <p class="text-sm text-slate-500 mt-0.5">Registra una torre dentro de una propiedad</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <TextInput id="name" bind:value={$form.name} placeholder="Ej: Torre A" error={$form.errors.name} />
                        </div>
                        <div>
                            <label for="floors" class="block text-sm font-medium text-slate-700 mb-1.5">
                                Número de pisos <span class="text-red-500">*</span>
                            </label>
                            <TextInput id="floors" type="number" bind:value={$form.floors} placeholder="10" error={$form.errors.floors} />
                        </div>
                    </div>
                    <div>
                        <label for="description" class="block text-sm font-medium text-slate-700 mb-1.5">Descripción</label>
                        <Textarea id="description" bind:value={$form.description} rows={2} placeholder="Información adicional de la torre..." error={$form.errors.description} />
                    </div>
            ...
            <div class="px-6 py-4 flex items-center justify-end gap-3 bg-slate-50">
                ...
                <Button type="submit" variant="primary" disabled={$form.processing}>
                    {$form.processing ? 'Guardando...' : 'Crear torre'}
                </Button>
            </div>
        </div>
    </form>
