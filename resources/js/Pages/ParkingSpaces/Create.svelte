<script>
    ...
    let { properties = [], towers = [], units = [] } = $props();
    ...
    // Filter towers/units based on selected property
...
<svelte:head>
    <title>Nuevo parqueadero — Portal PH</title>
</svelte:head>
...
<AppLayout>
    <div class="mb-6 flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-900">Nuevo parqueadero</h1>
            <p class="text-sm text-slate-500 mt-0.5">Registra un nuevo espacio de parqueo</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100 shadow-sm">
            ...
            <div class="px-6 py-5">
                ...
                <div>
                    ...
                    <TextInput id="code" bind:value={$form.code} placeholder="Ej: P-001, M-02, B-15" error={$form.errors.code} />
                    <p class="mt-1.5 text-xs text-slate-400">Debe ser único dentro de la propiedad.</p>
                </div>
            </div>
            ...
            <div class="px-6 py-4 flex items-center justify-end gap-3 bg-slate-50">
                ...
                <Button type="submit" variant="primary" disabled={$form.processing}>
                    {$form.processing ? 'Guardando...' : 'Crear parqueadero'}
                </Button>
            </div>
        </div>
    </form>
